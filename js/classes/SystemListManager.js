class SystemListManager {
    ...
    getCardHTML(system) {
        ...
        return `
            <div class="orbite-system-card-body">
                <div>
                    <span class="orbite-system-mode">${modeIcon} ${modeText}</span>
                    <h3>${system.name}</h3>
                </div>
                <div class="orbite-system-metrics">
                    <span><b>${system.players}</b> joueurs</span>
                    <span><b>${Math.floor(system.time/60)}</b> min</span>
                </div>
            </div>
        `;
    }
